 ...
 *
 * @remarks
 * `bun run check` runs every row in order and prints one line per row.
 * `bun run check:quick` runs the same rows without the slow ones, which is
 * what the push hook runs. `bun run check <row>` runs the named rows alone.
...
async function workflows(quick: boolean): Promise<string> {
  ...
  // mode with a warning the gate never prints. The input is .github,
  // which holds the workflows, dependabot.yml and any composite action under
  // .github/actions. --collect=all turns zizmor's ignore handling off, so no
  // .gitignore, exclude file or global excludes file can hide one of them, and
  // the input never reaches node_modules or .claude/worktrees. zizmor prints
...
// A committed generated file is checked by regenerating it in the tree and
// diffing. git diff reads the worktree against the index, so a stale index
...
async function markers(): Promise<string> {
  ...
  if (tracked.exitCode !== 0) {
    throw new Error(`git ls-files --error-unmatch ${describe(tracked)}`);
  }
  const scanned = await writeInventory();
  if (scanned.files === 0) {
...
// bun test exits 1 when it finds no test file, and 0 over a file holding no
// test or only skipped ones, so the row reads the count as scripts:test does.
// The root scripts directory is left to the scripts:test row.
async function test(): Promise<string> {
  const finished = await run([BUN, NO_ENV_FILE, 'test', '--coverage', '--path-ignore-patterns=scripts/**'], TEST_ENV);
...
// The rows that run the repository's own code come last: lint runs
// eslint.config.ts, and the two test rows run the tests. So every row that
...
const rows: readonly Row[] = [
  ...
  {
    name: 'markers',
    checks: `${INVENTORY} regenerated from the tree matches the index`,
    check: markers,
  },
  {
    name: 'lint',
...
async function main(): Promise<number> {
  if (process.argv.includes('--rows')) {
    ...
    for (const row of rows) {
      console.log(`  ${row.name.padEnd(width)}  ${row.checks}${row.slow === true ? dim(' (slow)') : ''}`);
    }
    return 0;
  }
